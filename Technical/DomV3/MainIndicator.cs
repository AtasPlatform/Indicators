...
public partial class MainIndicator : Indicator
{
	...
	private MarketDataArg? _lastAsk;
	private MarketDataArg? _lastBid;
	...
	protected override async void OnInitialize()
	{
		...
		_gridController = new MboGridController();
		_timer = new Timer();
	...
	protected override void OnMarketByOrdersChanged(IEnumerable<MarketByOrder> orders)
	{
		...
		if (!_gridController.Update(orders))
			_gridController.Load(MarketByOrders);
	}
	...
	protected override void MarketDepthChanged(MarketDataArg depth)
	{
		...
		if (!_gridController.Update(depth))
			_gridController.Load(MarketDepthInfo.GetMarketDepthSnapshot());
	}
	...
	protected override void OnRender(RenderContext context, DrawingLayouts layout)
	{
		try
		{
			// lock (_renderLock)
			{
				...
				var aggregationBaseRow = new Rectangle
					{ X = Container.RelativeRegion.Right - 1, Width = 0 };
