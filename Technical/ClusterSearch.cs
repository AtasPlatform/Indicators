    ...
    public class ClusterSearch : Indicator
    {
        ...
        protected override void OnCalculate(int bar, decimal value)
        {
            if (bar == 0)
            {
                ...
                _clusterTransColor = Color.FromArgb((byte)Math.Ceiling(255 * VisualObjectsTransparency * 0.01m), ClusterColor.R, ClusterColor.G, ClusterColor.B);
                _clusterPriceTransColor = Color.FromArgb((byte)Math.Ceiling(255 * Transparency * 0.01m), ClusterColor.R, ClusterColor.G, ClusterColor.B);
            }
            ...
            {
                for (var price = candle.Low; price <= candle.High; price += _tickSize)
                {
                    ...
                    for (var i = price; i < price + PriceRange * _tickSize; i += _tickSize)
                    {
                        ...
                        if ((candle.High - i) /_tickSize>= PipsFromHigh-1)
                            break;

                        if ((i - candle.Low)/_tickSize >= PipsFromLow-1)
                            break;

                        var priceInfo = candle.GetPriceVolumeInfo(i);

                        if (priceInfo != null && i != price)
                            sumInfo.Add(priceInfo);
                    }

                    if (price > candle.High || price < candle.Low)
                        continue;
            ...
            if (_isLastBar)
            {
                ...
                {
                    if (!_alertPrices.Contains(pair.Min))
                    {
                        _alertPrices.Add(pair.Min);
            ...
            foreach (var pair in _pairs)
            {
                var priceValue = new PriceSelectionValue(pair.Price)
                {
                    ...
                    MaximumPrice = pair.Max,
                    MinimumPrice = pair.Min,
                };
