namespace ATAS.Indicators.Technical
{
	...
	public class ClusterStatistic : Indicator
	{
		...
		private decimal _maxDeltaVolume;
		private decimal _maxExtDelta;
		private decimal _maxSessionDelta;
		private decimal _maxSessionDeltaVolume;
		...
		protected override void OnCalculate(int bar, decimal value)
		{
			...
			if (bar == 0)
			{
				_cumVolume = 0;
				_maxVolume = 0;
				_maxDelta = candle.Delta;
				_minDelta = candle.Delta;
				_maxAsk = candle.Ask;
				_maxBid = candle.Bid;
				_maxDeltaVolume = candle.Delta * 100.0m / candle.Volume;
				_maxSessionDelta = candle.Delta;
				_maxSessionDeltaVolume = _maxDeltaVolume;
				_minExtDelta = candle.MaxDelta;
				_maxExtDelta = candle.MinDelta;
				_maxDeltaChange = 0;
				_maxVolumeSec = 0;
		...
		protected override void OnRender(RenderContext context, DrawingLayouts layout)
		{
			...
			try
			{
				...
				var maxAsk = 0m;
				var maxBid = 0m;
				var maxDeltaVolume = 0m;
				var maxExtDelta = 0m;
				var minExtDelta = 0m;
				var maxDeltaChange = 0m;
				var maxVolumeSec = 0m;
				var maxcDelta = 0m;
				var maxcDeltaVolume = 0m;
				...
				else
				{
					...
					maxAsk = _maxAsk;
					maxBid = _maxBid;
					maxDeltaVolume = _maxDeltaVolume;
					maxExtDelta = _maxExtDelta;
					minExtDelta = _minExtDelta;
					maxDeltaChange = _maxDeltaChange;
					maxVolumeSec = _maxVolumeSec;
				}
				...
				for (var j = LastVisibleBarNumber; j >= FirstVisibleBarNumber; j--)
				{
					...
					if (ShowDeltaChange && j > 0)
					{
						...
						var rate = GetRate(Math.Abs(candle.Delta - prevCandle.Delta), maxDeltaChange);
						var bgBrush = Blend(VolumeColor, BackGroundColor, rate);
